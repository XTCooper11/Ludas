...
public:
    ...
    void Create(const char* label, float centerX, float centerY, float w, float h, TTF_Font* font) {
        // Center the main rectangle
        rect = { centerX - (w / 2.0f), centerY - (h / 2.0f), w, h };
        ...
        if (surf) {
            ...
            // Center text within the already centered button rect
            textRect = { rect.x + (w - surf->w) / 2.0f, rect.y + (h - surf->h) / 2.0f, (float)surf->w, (float)surf->h };
...
public:
    ...
    void Create(float centerX, float centerY, float w, float h, SDL_Color color) {
        // Center the bar background
        rect = { centerX - (w / 2.0f), centerY - (h / 2.0f), w, h };
        barColor = color;
    }
...
public:
    ...
    void Create(const char* text, float centerX, float centerY, TTF_Font* font, SDL_Color color = { 255,255,255,255 }) {
        if (tex) SDL_DestroyTexture(tex);

        SDL_Surface* surf = TTF_RenderText_Blended(font, text, 0, color);
        if (surf) {
            tex = SDL_CreateTextureFromSurface(renderer, surf);
            // Center based on surface dimensions
            rect = { centerX - (surf->w / 2.0f), centerY - (surf->h / 2.0f), (float)surf->w, (float)surf->h };
            SDL_DestroySurface(surf);
        }
    }
...
public:
    ...
    void Create(float centerX, float centerY, float w, float h, TTF_Font* font) {
    ...
    void Update(SDL_Event& event) {
        if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
            ...
            if (active && !wasActive) SDL_StartTextInput(window);
            else if (!active && wasActive) SDL_StopTextInput(window);
        }
    ...
    void Refresh() {
        ...
        if (surf) {
            ...
            float xOffset = 0;
            if (tw > availableWidth) xOffset = availableWidth - tw;
    ...
    void Render() {
        ...
        if (textTex && !text.empty()) SDL_RenderTexture(renderer, textTex, NULL, &textRect);
...
public:
    ...
    void Create(float centerX, float centerY, float w, float h, float startValue = 0.5f) {
        // Center the slider track
        rect = { centerX - (w / 2.0f), centerY - (h / 2.0f), w, h };
    ...
    void Update(SDL_Event& event) {
        ...
        if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN && SDL_PointInRectFloat(&mPos, &rect)) isDragging = true;
